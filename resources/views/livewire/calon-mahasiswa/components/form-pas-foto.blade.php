                ...
                <div class="row d-flex row-gap-4 flex-columns align-items-center">
                            ...
                            <div class="form-group">
                                <div class="position-relative d-flex justify-content-center align-items-center h-full w-full"
                                    ...
                                    @if ($pasFotoPreview || $uploadedPasFoto)
                                        <img class="rounded-4 w-full h-full object-fit-fill"
                    ...
                    @if ($pasFotoPreview || $uploadedPasFoto)
                        <div class="col-12 d-flex justify-content-center">
                            <span class="text-danger">Pas foto sudah tidak dapat diubah setelah melakukan
                                submit.</span>
                        </div>
                    @endif
                    <div class="col-12">
                        <div class="form-group w-100 d-flex justify-content-center">
                            <input class="form-control w-100 w-md-50" type="file" wire:model.live="pasFotoUpload"
                                {{ $pasFotoPreview || $uploadedPasFoto ? 'disabled' : '' }}>
                        </div>
                    </div>
                </div>
                <div class="col-12 d-flex justify-content-end mt-4">
                    <button wire:click="save" class="btn btn-primary me-1 mb-1" wire:confirm="Lanjutkan?"
                        {{ $pasFotoPreview || $uploadedPasFoto ? 'disabled' : '' }}>Submit</button>
                    <button type="reset" class="btn btn-danger me-1 mb-1"
                        {{ $pasFotoPreview || $uploadedPasFoto ? 'disabled' : '' }}>Reset</button>
                </div>
            </section>
        </div>
    </div>
